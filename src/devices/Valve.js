...
export class Valve {
  ...
  handleAttributeUpdate(accessory, update) {
    ...
    this.logManager.logDebug(
      `Valve | ${accessory.displayName} | Attribute update: ${attribute} = ${value}`,
    );
    if (!Valve.relevantAttributes.includes(attribute)) return;

    const svc = accessory.getService(
      this.Service.Valve,
      this.generateSrvcName(accessory.displayName, "Valve"),
    );
    if (!svc) return;
    ...
    switch (attribute) {
      case "valve":
        svc
          ...
          .updateValue(this._getInUseState(value));
        svc
      ...
      default:
        this.logManager.logWarn(
          `Valve | ${accessory.displayName} | Unhandled attribute update: ${attribute} = ${value}`,
        );
    }
  }
}

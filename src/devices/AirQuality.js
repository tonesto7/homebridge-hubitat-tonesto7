...
export class AirQuality {
  ...
  configure(accessory) {
    this.logManager.logDebug(
      `Configuring Air Quality Sensor for ${accessory.displayName}`,
    );
  ...
  _configureAirQuality(accessory, svc, devData) {
    accessory.getOrAddCharacteristic(svc, this.Characteristic.AirQuality, {
      getHandler: () =>
        this._getAirQualityState(devData.attributes.airQualityIndex),
    });
  }
  ...
  handleAttributeUpdate(accessory, update) {
    ...
    this.logManager.logDebug(
      `AirQuality | ${accessory.displayName} | Attribute update: ${attribute} = ${value}`,
    );
    if (!AirQuality.relevantAttributes.includes(attribute)) return;

    const svc = accessory.getService(
      this.Service.AirQualitySensor,
      this.generateSrvcName(accessory.displayName, "Air Quality"),
    );
    if (!svc) return;

    svc
      .getCharacteristic(this.Characteristic.StatusActive)
      .updateValue(this._getStatusActive(accessory.context.deviceData.status));

    switch (attribute) {
      case "airQualityIndex":
        svc
          .getCharacteristic(this.Characteristic.AirQuality)
          .updateValue(this._getAirQualityState(value));
        break;
      case "battery":
        svc
          .getCharacteristic(this.Characteristic.StatusLowBattery)
          .updateValue(this._getStatusLowBattery(value));
        break;
      case "pm25":
        svc
          .getCharacteristic(this.Characteristic.PM2_5Density)
          .updateValue(this._getPM25Density(value));
        break;
      case "tamper":
        svc
          .getCharacteristic(this.Characteristic.StatusTampered)
          .updateValue(this._getStatusTampered(value));
        break;
      default:
        this.logManager.logWarn(
          `AirQuality | ${accessory.displayName} | Unhandled attribute update: ${attribute} = ${value}`,
        );
    }
  }
}

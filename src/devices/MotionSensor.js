...
export class MotionSensor {
    ...
    configure(accessory) {
        ...
        const svc = accessory.getOrAddService(this.Service.MotionSensor, this.generateSrvcName(accessory.displayName, "Motion"));
    ...
    handleAttributeUpdate(accessory, update) {
        ...
        switch (attribute) {
            case "motion":
                svc.getCharacteristic(this.Characteristic.MotionDetected).updateValue(this._getMotionState(value));
                break;
            case "tamper":
                svc.getCharacteristic(this.Characteristic.StatusTampered).updateValue(this._getTamperedState(value));
                break;
            default:
                this.logManager.logWarn(`MotionSensor | ${accessory.displayName} | Unhandled attribute update: ${attribute} = ${value}`);
        }
    }
}

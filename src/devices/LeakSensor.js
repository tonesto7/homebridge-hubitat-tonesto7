...
export class LeakSensor {
  ...
  configure(accessory) {
    this.logManager.logDebug(
      `Configuring Leak Sensor for ${accessory.displayName}`,
    );
  ...
  handleAttributeUpdate(accessory, update) {
    ...
    this.logManager.logDebug(
      `LeakSensor | ${accessory.displayName} | Attribute update: ${attribute} = ${value}`,
    );
    if (!LeakSensor.relevantAttributes.includes(attribute)) return;

    const svc = accessory.getService(
      this.Service.LeakSensor,
      this.generateSrvcName(accessory.displayName, "Leak"),
    );
    if (!svc) return;
    ...
    switch (attribute) {
      case "water":
        svc
          ...
          .updateValue(this._getLeakState(value));
        break;
      case "tamper":
        svc
      ...
      default:
        this.logManager.logWarn(
          `LeakSensor | ${accessory.displayName} | Unhandled attribute update: ${attribute} = ${value}`,
        );
    }
  }
}

...
export class EnergyMeter {
  ...
  configure(accessory) {
    this.logManager.logDebug(
      `Configuring Energy Meter for ${accessory.displayName}`,
    );
    const svc = accessory.getOrAddService(
      this.CommunityTypes.KilowattHoursService,
  ...
  handleAttributeUpdate(accessory, update) {
    ...
    this.logManager.logDebug(
      `EnergyMeter | ${accessory.displayName} | Attribute update: ${attribute} = ${value}`,
    );
    if (!EnergyMeter.relevantAttributes.includes(attribute)) return;

    const svc = accessory.getService(
      this.CommunityTypes.KilowattHoursService,
      this.generateSrvcName(accessory.displayName, "Energy"),
    );
    if (!svc) return;
    switch (attribute) {
      case "energy":
        svc
          .getCharacteristic(this.CommunityTypes.KilowattHours)
          .updateValue(this._getEnergyValue(value));
        break;
      default:
        this.logManager.logWarn(
          `EnergyMeter | ${accessory.displayName} | Unhandled attribute update: ${attribute} = ${value}`,
        );
    }
  }
}

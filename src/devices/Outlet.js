...
export class Outlet {
  ...
  handleAttributeUpdate(accessory, update) {
    ...
    this.logManager.logDebug(
      `Outlet | ${accessory.displayName} | Attribute update: ${attribute} = ${value}`,
    );
    if (!Outlet.relevantAttributes.includes(attribute)) return;

    const svc = accessory.getService(
      this.Service.Outlet,
      this.generateSrvcName(accessory.displayName, "Outlet"),
    );
    if (!svc) return;
    ...
    switch (attribute) {
      ...
      default:
        this.logManager.logWarn(
          `Outlet | ${accessory.displayName} | Unhandled attribute update: ${attribute} = ${value}`,
        );
    }
  }
}

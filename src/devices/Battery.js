...
export class Battery {
  ...
  configure(accessory) {
    this.logManager.logDebug(
      `Configuring Battery for ${accessory.displayName}`,
    );
  ...
  handleAttributeUpdate(accessory, update) {
    ...
    this.logManager.logDebug(
      `Battery | ${accessory.displayName} | Attribute update: ${attribute} = ${value}`,
    );
    if (!Battery.relevantAttributes.includes(attribute)) return;

    const svc = accessory.getService(
      this.Service.Battery,
      this.generateSrvcName(accessory.displayName, "Battery"),
    );
    if (!svc) return;
    ...
    switch (attribute) {
      case "battery":
        svc
          ...
          .updateValue(this._getBatteryLevel(value));
        break;
      case "powerSource":
        svc
          .getCharacteristic(this.Characteristic.ChargingState)
          .updateValue(this._getChargingState(value));
        break;
      default:
        this.logManager.logWarn(
          `Battery | ${accessory.displayName} | Unhandled attribute update: ${attribute} = ${value}`,
        );
    }
  }
}

...
export class HumiditySensor {
  ...
  configure(accessory) {
    this.logManager.logDebug(
      `Configuring Humidity Sensor for ${accessory.displayName}`,
    );
  ...
  handleAttributeUpdate(accessory, update) {
    ...
    this.logManager.logDebug(
      `HumiditySensor | ${accessory.displayName} | Attribute update: ${attribute} = ${value}`,
    );
    if (!HumiditySensor.relevantAttributes.includes(attribute)) return;

    const svc = accessory.getService(
      this.Service.HumiditySensor,
      this.generateSrvcName(accessory.displayName, "Humidity"),
    );
    if (!svc) return;
    ...
    switch (attribute) {
      case "humidity":
        svc
          ...
          .updateValue(this._getCurrentHumidity(value));
        break;
      case "tamper":
        svc
      ...
      default:
        this.logManager.logWarn(
          `HumiditySensor | ${accessory.displayName} | Unhandled attribute update: ${attribute} = ${value}`,
        );
    }
  }
}

...
export class ContactSensor {
  ...
  configure(accessory) {
    this.logManager.logDebug(
      `Configuring Contact Sensor for ${accessory.displayName}`,
    );
  ...
  handleAttributeUpdate(accessory, update) {
    ...
    this.logManager.logDebug(
      `ContactSensor | ${accessory.displayName} | Attribute update: ${attribute} = ${value}`,
    );
    if (!ContactSensor.relevantAttributes.includes(attribute)) return;

    const svc = accessory.getService(
      this.Service.ContactSensor,
      this.generateSrvcName(accessory.displayName, "Contact"),
    );
    if (!svc) return;
    ...
    switch (attribute) {
      case "contact":
        svc
          ...
          .updateValue(this._getContactState(value));
        break;
      case "tamper":
        svc
      ...
      default:
        this.logManager.logWarn(
          `ContactSensor | ${accessory.displayName} | Unhandled attribute update: ${attribute} = ${value}`,
        );
    }
  }
}

...
export class GarageDoor {
  ...
  configure(accessory) {
    this.logManager.logDebug(
      `Configuring Garage Door for ${accessory.displayName}`,
    );
    ...
    accessory.getOrAddCharacteristic(svc, this.Characteristic.TargetDoorState, {
      ...
      setHandler: (value) =>
        accessory.sendCommand(
          value === this.Characteristic.TargetDoorState.OPEN ? "open" : "close",
        ),
    });
  }
  ...
  handleAttributeUpdate(accessory, update) {
    ...
    this.logManager.logDebug(
      `GarageDoor | ${accessory.displayName} | Attribute update: ${attribute} = ${value}`,
    );
    if (!GarageDoor.relevantAttributes.includes(attribute)) return;

    const svc = accessory.getService(
      this.Service.GarageDoorOpener,
      this.generateSrvcName(accessory.displayName, "Garage Door"),
    );
    if (!svc) return;
    ...
    switch (attribute) {
      case "door":
        svc
          ...
          .updateValue(this._getCurrentDoorState(value));
        break;
      default:
        this.logManager.logWarn(
          `GarageDoor | ${accessory.displayName} | Unhandled attribute update: ${attribute} = ${value}`,
        );
    }
  }
}

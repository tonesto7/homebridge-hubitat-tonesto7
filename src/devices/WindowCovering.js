...
export class WindowCovering {
  ...
  configure(accessory) {
    this.logManager.logDebug(
      `Configuring Window Covering for ${accessory.displayName}`,
    );
    ...
    const positionAttr = accessory.hasCommand("setPosition")
      ? "position"
      : "level";
  ...
  _configureCurrentPosition(accessory, svc, devData, positionAttr) {
    accessory.getOrAddCharacteristic(svc, this.Characteristic.CurrentPosition, {
      getHandler: () =>
        this._getCurrentPosition(devData.attributes[positionAttr]),
  ...
  _configureTargetPosition(accessory, svc, devData, positionAttr) {
    accessory.getOrAddCharacteristic(svc, this.Characteristic.TargetPosition, {
      getHandler: () =>
        this._getTargetPosition(parseInt(devData.attributes[positionAttr])),
      setHandler: (value) => {
        if (accessory.hasCommand("close") && value <= 2) {
          accessory.sendCommand("close");
        } else {
          const v = this._getTargetPosition(value);
          accessory.sendCommand(
            accessory.hasCommand("setPosition") ? "setPosition" : "setLevel",
            [v],
          );
        }
      },
  ...
  _configureObstruction(accessory, svc) {
    svc
      ...
      .setCharacteristic(this.Characteristic.HoldPosition, false);
  }
  ...
  handleAttributeUpdate(accessory, update) {
    ...
    this.logManager.logDebug(
      `WindowCovering | ${accessory.displayName} | Attribute update: ${attribute} = ${value}`,
    );
    if (!WindowCovering.relevantAttributes.includes(attribute)) return;

    const svc = accessory.getService(
      this.Service.WindowCovering,
      this.generateSrvcName(accessory.displayName, "Shade"),
    );
    if (!svc) return;
    ...
    switch (attribute) {
      case "windowShade":
        svc
          ...
          .updateValue(this._getPositionState(value));
        break;
      case "position":
      case "level":
        svc
          .getCharacteristic(this.Characteristic.CurrentPosition)
          .updateValue(this._getCurrentPosition(value));
        break;
      default:
        this.logManager.logWarn(
          `WindowCovering | ${accessory.displayName} | Unhandled attribute update: ${attribute} = ${value}`,
        );
    }
  }
}

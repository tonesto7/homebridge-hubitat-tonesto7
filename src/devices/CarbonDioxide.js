...
export class CarbonDioxide {
  ...
  configure(accessory) {
    this.logManager.logDebug(
      `Configuring CO2 Sensor for ${accessory.displayName}`,
    );
  ...
  _configureCo2Detected(accessory, svc, devData) {
    accessory.getOrAddCharacteristic(
      ...
      {
        getHandler: () =>
          this._getCo2DetectedState(devData.attributes.carbonDioxide),
      },
    );
  }
  ...
  _configureCo2Level(accessory, svc, devData) {
    accessory.getOrAddCharacteristic(
      svc,
      this.Characteristic.CarbonDioxideLevel,
      {
        getHandler: () =>
          this._getCo2Level(devData.attributes.carbonDioxideMeasurement),
      },
    );
  }
  ...
  _getCo2DetectedState(value) {
    if (!value || isNaN(value))
      return this.Characteristic.CarbonDioxideDetected.CO2_LEVELS_NORMAL;
    return value < 2000
  ...
  handleAttributeUpdate(accessory, update) {
    ...
    this.logManager.logDebug(
      `CarbonDioxide | ${accessory.displayName} | Attribute update: ${attribute} = ${value}`,
    );
    if (!CarbonDioxide.relevantAttributes.includes(attribute)) return;

    const svc = accessory.getService(
      this.Service.CarbonDioxideSensor,
      this.generateSrvcName(accessory.displayName, "CO2"),
    );
    if (!svc) return;

    svc
      .getCharacteristic(this.Characteristic.StatusActive)
      .updateValue(this._getActiveState(accessory.context.deviceData.status));

    switch (attribute) {
      case "carbonDioxide":
        svc
          .getCharacteristic(this.Characteristic.CarbonDioxideLevel)
          .updateValue(this._getCo2Level(value));
        break;
      case "tamper":
        svc
      ...
      default:
        this.logManager.logWarn(
          `CarbonDioxide | ${accessory.displayName} | Unhandled attribute update: ${attribute} = ${value}`,
        );
    }
  }
}

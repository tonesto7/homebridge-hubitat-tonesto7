...
export class AccelerationSensor {
  ...
  configure(accessory) {
    this.logManager.logDebug(
      `Configuring Acceleration Sensor for ${accessory.displayName}`,
    );
  ...
  _configureMotionDetected(accessory, svc, devData) {
    accessory.getOrAddCharacteristic(svc, this.Characteristic.MotionDetected, {
      getHandler: () =>
        this._getMotionDetected(devData.attributes.acceleration),
    });
  }
  ...
  handleAttributeUpdate(accessory, update) {
    ...
    this.logManager.logDebug(
      `AccelerationSensor | ${accessory.displayName} | Attribute update: ${attribute} = ${value}`,
    );

    const svc = accessory.getService(this.Service.MotionSensor);
    if (!svc) {
      this.logManager.logWarn(
        `AccelerationSensor | ${accessory.displayName} | No service found`,
      );
      return;
    }
    ...
    switch (attribute) {
      case "acceleration":
        svc
          ...
          .updateValue(this._getMotionDetected(value));
        break;
      case "tamper":
        svc
      ...
      default:
        this.logManager.logWarn(
          `AccelerationSensor | ${accessory.displayName} | Unhandled attribute update: ${attribute} = ${value}`,
        );
    }
  }
}

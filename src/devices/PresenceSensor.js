...
export class PresenceSensor {
  ...
  configure(accessory) {
    this.logManager.logDebug(
      `Configuring Presence Sensor for ${accessory.displayName}`,
    );
  ...
  handleAttributeUpdate(accessory, update) {
    ...
    this.logManager.logDebug(
      `PresenceSensor | ${accessory.displayName} | Attribute update: ${attribute} = ${value}`,
    );
    if (!PresenceSensor.relevantAttributes.includes(attribute)) return;

    const svc = accessory.getService(
      this.Service.OccupancySensor,
      this.generateSrvcName(accessory.displayName, "Presence"),
    );
    if (!svc) return;
    ...
    switch (attribute) {
      case "presence":
        svc
          ...
          .updateValue(this._getPresenceState(value));
        break;
      case "tamper":
        svc
      ...
      default:
        this.logManager.logWarn(
          `PresenceSensor | ${accessory.displayName} | Unhandled attribute update: ${attribute} = ${value}`,
        );
    }
  }
}

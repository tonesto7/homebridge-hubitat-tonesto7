...
export class Lock {
    ...
    accessory.getOrAddCharacteristic(svc, this.Characteristic.LockTargetState, {
      ...
      setHandler: (value) =>
        accessory.sendCommand(
          value === this.Characteristic.LockTargetState.SECURED
            ? "lock"
            : "unlock",
        ),
    });
  ...
  handleAttributeUpdate(accessory, update) {
    ...
    this.logManager.logDebug(
      `Lock | ${accessory.displayName} | Attribute update: ${attribute} = ${value}`,
    );
    if (!Lock.relevantAttributes.includes(attribute)) return;

    const svc = accessory.getService(
      this.Service.LockMechanism,
      this.generateSrvcName(accessory.displayName, "Lock"),
    );
    if (!svc) return;
    ...
    switch (attribute) {
      case "lock":
        svc
          ...
          .updateValue(this._getLockCurrentState(value));
        break;
      default:
        this.logManager.logWarn(
          `Lock | ${accessory.displayName} | Unhandled attribute update: ${attribute} = ${value}`,
        );
    }
  }
}

...
export class CarbonMonoxide {
  ...
  configure(accessory) {
    this.logManager.logDebug(
      `Configuring CO Sensor for ${accessory.displayName}`,
    );
  ...
  _configureCoDetected(accessory, svc, devData) {
    accessory.getOrAddCharacteristic(
      ...
      {
        getHandler: () =>
          this._getCoDetectedState(devData.attributes.carbonMonoxide),
      },
    );
  }
  ...
  handleAttributeUpdate(accessory, update) {
    ...
    this.logManager.logDebug(
      `CarbonMonoxide | ${accessory.displayName} | Attribute update: ${attribute} = ${value}`,
    );
    if (!CarbonMonoxide.relevantAttributes.includes(attribute)) return;

    const svc = accessory.getService(
      this.Service.CarbonMonoxideSensor,
      this.generateSrvcName(accessory.displayName, "CO"),
    );
    if (!svc) return;
    ...
    switch (attribute) {
      case "carbonMonoxide":
        svc
          ...
          .updateValue(this._getCoDetectedState(value));
        break;
      case "tamper":
        svc
      ...
      default:
        this.logManager.logWarn(
          `CarbonMonoxide | ${accessory.displayName} | Unhandled attribute update: ${attribute} = ${value}`,
        );
    }
  }
}

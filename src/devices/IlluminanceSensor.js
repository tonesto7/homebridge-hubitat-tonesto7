...
export class IlluminanceSensor {
  ...
  configure(accessory) {
    this.logManager.logDebug(
      `Configuring Illuminance Sensor for ${accessory.displayName}`,
    );
  ...
  _configureAmbientLight(accessory, svc, devData) {
    accessory.getOrAddCharacteristic(
      ...
      {
        getHandler: () =>
          this._getCurrentAmbientLightLevel(devData.attributes.illuminance),
  ...
  handleAttributeUpdate(accessory, update) {
    ...
    this.logManager.logDebug(
      `IlluminanceSensor | ${accessory.displayName} | Attribute update: ${attribute} = ${value}`,
    );
    if (!IlluminanceSensor.relevantAttributes.includes(attribute)) return;

    const svc = accessory.getService(
      this.Service.LightSensor,
      this.generateSrvcName(accessory.displayName, "Illuminance"),
    );
    if (!svc) return;
    ...
    switch (attribute) {
      case "illuminance":
        svc
          ...
          .updateValue(this._getCurrentAmbientLightLevel(value));
        break;
      case "tamper":
        svc
      ...
      default:
        this.logManager.logWarn(
          `IlluminanceSensor | ${accessory.displayName} | Unhandled attribute update: ${attribute} = ${value}`,
        );
    }
  }
}

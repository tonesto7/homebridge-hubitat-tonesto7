...
export class AirPurifier {
  ...
  configure(accessory) {
    this.logManager.logDebug(
      `Configuring Air Purifier for ${accessory.displayName}`,
    );
  ...
  _configureFanOscillationMode(accessory, svc, devData) {
    accessory.getOrAddCharacteristic(
      svc,
      this.CommunityTypes.FanOscillationMode,
      {
        getHandler: () => {
  ...
  async handleAttributeUpdate(accessory, update) {
    ...
    this.logManager.logDebug(
      `AirPurifier | ${accessory.displayName} | Attribute update: ${attribute} = ${value}`,
    );
    ...
    if (!svc) {
      this.logManager.logWarn(
        `AirPurifier | ${accessory.displayName} | No service found`,
      );
      return;
    }
    ...
    switch (attribute) {
      case "switch":
        svc
          ...
          .updateValue(this._getActiveState(value));
        break;
      case "fanMode":
        svc
          .getCharacteristic(this.CommunityTypes.FanOscillationMode)
          .updateValue(this._getFanOscillationMode(value));
        break;
    }
  }
}

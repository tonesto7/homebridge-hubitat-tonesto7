...
export class VirtualMode {
  ...
  configure(accessory) {
    this.logManager.logDebug(
      `Configuring Virtual Mode for ${accessory.displayName}`,
    );
  ...
  handleAttributeUpdate(accessory, update) {
    ...
    this.logManager.logDebug(
      `VirtualMode | ${accessory.displayName} | Attribute update: ${attribute} = ${value}`,
    );
    if (!VirtualMode.relevantAttributes.includes(attribute)) return;

    const svc = accessory.getService(
      this.Service.Switch,
      this.generateSrvcName(accessory.displayName, "Mode"),
    );
    if (!svc) return;
    ...
    switch (attribute) {
      case "switch":
        svc
          ...
          .updateValue(this._getOnState(value));
        break;
      default:
        this.logManager.logWarn(
          `VirtualMode | ${accessory.displayName} | Unhandled attribute update: ${attribute} = ${value}`,
        );
    }
  }
}

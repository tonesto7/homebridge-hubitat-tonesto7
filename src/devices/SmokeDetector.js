...
export class SmokeDetector {
  ...
  configure(accessory) {
    this.logManager.logDebug(
      `Configuring Smoke Detector for ${accessory.displayName}`,
    );
  ...
  handleAttributeUpdate(accessory, update) {
    ...
    this.logManager.logDebug(
      `SmokeDetector | ${accessory.displayName} | Attribute update: ${attribute} = ${value}`,
    );
    if (!SmokeDetector.relevantAttributes.includes(attribute)) return;

    const svc = accessory.getService(
      this.Service.SmokeSensor,
      this.generateSrvcName(accessory.displayName, "Smoke"),
    );
    if (!svc) return;
    ...
    switch (attribute) {
      case "smoke":
        svc
          ...
          .updateValue(this._getSmokeState(value));
        break;
      case "tamper":
        svc
      ...
      default:
        this.logManager.logWarn(
          `SmokeDetector | ${accessory.displayName} | Unhandled attribute update: ${attribute} = ${value}`,
        );
    }
  }
}

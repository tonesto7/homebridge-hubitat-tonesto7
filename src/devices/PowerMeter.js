...
export class PowerMeter {
  ...
  configure(accessory) {
    this.logManager.logDebug(
      `Configuring Power Meter for ${accessory.displayName}`,
    );
    const svc = accessory.getOrAddService(
      this.CommunityTypes.WattService,
  ...
  handleAttributeUpdate(accessory, update) {
    ...
    this.logManager.logDebug(
      `PowerMeter | ${accessory.displayName} | Attribute update: ${attribute} = ${value}`,
    );
    if (!PowerMeter.relevantAttributes.includes(attribute)) return;

    const svc = accessory.getService(
      this.CommunityTypes.WattService,
      this.generateSrvcName(accessory.displayName, "Power"),
    );
    if (!svc) return;
    ...
    switch (attribute) {
      case "power":
        svc
          .getCharacteristic(this.CommunityTypes.Watts)
          .updateValue(this._getPowerValue(value));
        break;
      default:
        this.logManager.logWarn(
          `PowerMeter | ${accessory.displayName} | Unhandled attribute update: ${attribute} = ${value}`,
        );
    }
  }
}

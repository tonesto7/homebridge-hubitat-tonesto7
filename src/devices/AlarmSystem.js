...
export class AlarmSystem {
  ...
  configure(accessory) {
    this.logManager.logDebug(
      `Configuring Alarm System for ${accessory.displayName}`,
    );
  ...
  _configureCurrentState(accessory, svc, devData) {
    accessory.getOrAddCharacteristic(
      ...
      {
        getHandler: () =>
          this._getSecuritySystemCurrentState(
            devData.attributes.alarmSystemStatus,
          ),
      },
    );
  }
  ...
  _configureTargetState(accessory, svc, devData) {
    accessory.getOrAddCharacteristic(
      ...
      {
        getHandler: () =>
          this._getSecuritySystemTargetState(
            devData.attributes.alarmSystemStatus,
          ),
        setHandler: (value) => {
  ...
  handleAttributeUpdate(accessory, update) {
    ...
    this.logManager.logDebug(
      `AlarmSystem | ${accessory.displayName} | Attribute update: ${attribute} = ${value}`,
    );
    if (!AlarmSystem.relevantAttributes.includes(attribute)) return;

    const svc = accessory.getService(
      this.Service.SecuritySystem,
      this.generateSrvcName(accessory.displayName, "Alarm System"),
    );
    if (!svc) return;
    ...
    switch (attribute) {
      case "alarmSystemStatus":
        svc
          ...
          .updateValue(this._getSecuritySystemCurrentState(value));
        break;
      default:
        this.logManager.logWarn(
          `AlarmSystem | ${accessory.displayName} | Unhandled attribute update: ${attribute} = ${value}`,
        );
    }
  }
}

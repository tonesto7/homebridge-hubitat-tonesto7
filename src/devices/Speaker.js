...
export class Speaker {
  ...
  configure(accessory) {
    this.logManager.logDebug(
      `Configuring Speaker for ${accessory.displayName}`,
    );
    ...
    this.levelAttr =
      this.isSonos || accessory.hasAttribute("volume")
        ? "volume"
        : accessory.hasAttribute("level")
          ? "level"
          : undefined;
      ...
      setHandler: (value) => {
        if (this._validateVolume(value)) {
          accessory.sendCommand(
            this.isSonos
              ? "setVolume"
              : `set${this.levelAttr.charAt(0).toUpperCase() + this.levelAttr.slice(1)}`,
            [value],
          );
        } else {
  ...
  handleAttributeUpdate(accessory, update) {
    ...
    this.logManager.logDebug(
      `Speaker | ${accessory.displayName} | Attribute update: ${attribute} = ${value}`,
    );
    if (!Speaker.relevantAttributes.includes(attribute)) return;

    const svc = accessory.getService(
      this.Service.Speaker,
      this.generateSrvcName(accessory.displayName, "Speaker"),
    );
    if (!svc) return;
    ...
    switch (attribute) {
      ...
      case "level":
        svc
          ...
          .updateValue(this._getVolume(value));
        break;
      case "mute":
        svc
          .getCharacteristic(this.Characteristic.Mute)
          .updateValue(this._getMuteState(value));
        break;
      default:
        this.logManager.logWarn(
          `Speaker | ${accessory.displayName} | Unhandled attribute update: ${attribute} = ${value}`,
        );
    }
  }
}

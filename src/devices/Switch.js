...
export class Switch {
  ...
  handleAttributeUpdate(accessory, update) {
    ...
    this.logManager.logDebug(
      `Switch | ${accessory.displayName} | Attribute update: ${attribute} = ${value}`,
    );
    if (!Switch.relevantAttributes.includes(attribute)) return;

    const svc = accessory.getService(
      this.Service.Switch,
      this.generateSrvcName(accessory.displayName, "Switch"),
    );
    if (!svc) return;
    ...
    switch (attribute) {
      ...
      default:
        this.logManager.logWarn(
          `Switch | ${accessory.displayName} | Unhandled attribute update: ${attribute} = ${value}`,
        );
    }
  }
}

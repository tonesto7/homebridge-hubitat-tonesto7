    ...
    if (device && device.capabilities) {
        ...
        if (isSpeaker === true) {
            ...
            thisCharacteristic = that.getaddService(Service.Speaker).getCharacteristic(Characteristic.Volume)
                .on('get', function(callback) {
                    callback(null, parseInt(device.attributes.level || 0));
                })
                .on('set', function(value, callback) {
                    if (value > 0) {
                        platform.api.runCommand(callback, device.deviceid, 'setLevel', {
            ...
            thisCharacteristic = that.getaddService(Service.Speaker).getCharacteristic(Characteristic.Mute)
                .on('get', function(callback) {
                    callback(null, device.attributes.mute === 'muted');
                })
                .on('set', function(value, callback) {
                    if (value) {
        ...
        if (isFan === true && (device.capabilities['Fan Light'] !== undefined || device.capabilities['FanLight'] !== undefined || that.deviceGroup === 'unknown')) {
            ...
            if (device.attributes.level !== undefined || device.attributes.fanSpeed !== undefined) {
                let fanLvl = device.attributes.fanSpeed ? fanSpeedConversion(device.attributes.fanSpeed, (device.command['medHighSpeed'] !== undefined)) : parseInt(device.attributes.level);
                platform.log("Fan with (" + device.attributes.fanSpeed ? "fanSpeed" : "level" + ') | value: ' + fanLvl);
                thisCharacteristic = that.getaddService(Service.Fanv2).getCharacteristic(Characteristic.RotationSpeed)
                    ...
                    .on('set', function(value, callback) {
                        if (value > 0) {
                            let cmdStr = (device.attributes.fanSpeed) ? 'fanspeed' : 'setLevel';
                            let cmdVal = (device.attributes.fanSpeed) ? fanSpeedConversion(value, (device.command['medHighSpeed'] !== undefined)) : parseInt(value);
        ...
        if (isMode === true) {
            ...
            thisCharacteristic = that.getaddService(Service.Switch).getCharacteristic(Characteristic.On)
                ...
                .on('set', function(value, callback) {
                    if (value && device.attributes.switch === 'off') {
        ...
        // Smoke Detectors
        if ((device.capabilities['Smoke Detector'] !== undefined || device.capabilities['SmokeDetector'] !== undefined) && device.attributes.smoke) {
        ...
        if ((device.capabilities['Carbon Monoxide Detector'] !== undefined || device.capabilities['CarbonMonoxideDetector'] !== undefined) && device.attributes.carbonMonoxide) {
            that.deviceGroup = 'detectors';
        ...
        if ((device.capabilities['Carbon Dioxide Measurement'] !== undefined || device.capabilities['CarbonDioxideMeasurement'] !== undefined) && device.attributes.carbonDioxideMeasurement) {
        ...
        //     .on('get', function(callback) {
        //         callback(null, Math.round(device.attributes.energy));
        //     });
        //     platform.addAttributeUsage('energy', device.deviceid, thisCharacteristic);
        // }
                ...
                .on('set', function(value, callback) {
                    switch (value) {
                        case Characteristic.TargetHeatingCoolingState.COOL:
                            ...
                            device.attributes.thermostatMode = 'cool';
                            break;
                        case Characteristic.TargetHeatingCoolingState.HEAT:
                            platform.api.runCommand(callback, device.deviceid, 'heat');
                            device.attributes.thermostatMode = 'heat';
                            break;
                        case Characteristic.TargetHeatingCoolingState.AUTO:
                            platform.api.runCommand(callback, device.deviceid, 'auto');
                            device.attributes.thermostatMode = 'auto';
                            break;
                        case Characteristic.TargetHeatingCoolingState.OFF:
                            platform.api.runCommand(callback, device.deviceid, 'off');
                            device.attributes.thermostatMode = 'off';
                            break;
                    }
                });
                ...
                .on('set', function(value, callback) {
                    ...
                    switch (device.attributes.thermostatMode) {
                        case 'cool':
                            ...
                            device.attributes.coolingSetpoint = temp;
                            break;
                        case 'emergency heat':
                        case 'heat':
                            ...
                            device.attributes.heatingSetpoint = temp;
                            break;
                        default:
                            // This should only refer to auto
                            // Choose closest target as single target
        ...
        if (device.attributes['alarmSystemStatus'] !== undefined) {
            ...
            thisCharacteristic = that.getaddService(Service.SecuritySystem).getCharacteristic(Characteristic.SecuritySystemTargetState)
                ...
                .on('set', function(value, callback) {
                    // platform.log(device.deviceid + ' set value : ' + value);
                    let val = convertAlarmState(value);
                    platform.api.runCommand(callback, device.deviceid, val);
                    device.attributes.alarmSystemStatus = val;
                });
            platform.addAttributeUsage('alarmSystemStatus', device.deviceid, thisCharacteristic);
        }
    }
